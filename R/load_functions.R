...
read_border_crossing_data <- function(config, directory, data_col = 4) {
  ...
  output <- left_join(
    output,
    data %>% filter(year == 2019) %>% select(c("day", "col_")),
    by = c("day"),
    suffix = c("1", "2")
  )
...
read_hlfs_data <- function(config, directory, odata_definitions=NULL) {
  ...
  for (i in 1:length(config$filename)) {
    data <- as.data.frame(suppressMessages(read_excel(paste0(directory, config$filename[i]), sheet = config$sheet_number, col_names = TRUE, skip = skip, .name_repair = "unique")))
    ...
    quarter <- as.data.frame(suppressMessages(read_excel(paste0(directory, config$filename[i]), sheet = config$sheet_number, range = "A4", col_names = FALSE)))
