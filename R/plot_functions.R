get_time_series_plot <- function(data_object, input, indicator_definition, type = "line", stacking = "normal") {
  ...
  if (!is.null(indicator_definition$include_date_slider) && indicator_definition$include_date_slider) {
  ...
  plot <- highchart()
  plot <- hc_exporting(plot, enabled = TRUE, filename = paste0(input$indicator_selector), buttons = list(
    contextButton = list(
  ...
  if ((abs(as.numeric(difftime(dates[[1]], dates[[length(dates)]], units = c("days")))) < 7) & length(dates) > 7) {
  ...
  } else if ((abs(as.numeric(difftime(dates[[1]], dates[[length(dates)]], units = c("days")))) < 360)) {
  ...
  plot <- plot %>%
    hc_yAxis(
      title = list(text = paste(y_label, norm_factor_and_unit$unit),
                   style = list(fontSize = "20px",  color = "black", fontFamily = "Source Sans Pro")), labels = list(style = list(
                     fontSize = "20px", color = "black", fontFamily = "Source Sans Pro"
                   ))) %>%
    hc_add_theme(
      hc_theme(
    ...
    hc_tooltip(table = TRUE,
               ...
               pointFormat = paste0( '<br> <span style="color:{point.color}">\u25CF</span>',
                                     " {series.name}: ", tool_tip$prefix , "{point.y} ", norm_factor_and_unit$unit, tool_tip$suffix),
...
get_bar_chart <- function(data_object, input, indicator_definition, type, rotation, stacking = "normal") {
  plot <- highchart()
  group_index <- which(sapply(indicator_definition$groups, function(x) x$name) == input$line_selector)
  group_definition <- indicator_definition$groups[[group_index]]
  plot <- highchart()
...
get_unstacked_vertical_bar <- function(data, input, indicator_definition) {
  get_bar_chart(data, input, indicator_definition, type = "column", rotation = -45, stacking = NULL)
}
...
get_time_series_plot_with_errors <- function(data_object, input, indicator_definition, type = "line", stacking = "normal") {
  ...
  for (i in 1:length(data_object$value_names)) {
    time_series_data <- data_object$values[,i][time_range_index]
    error_limits <- as.data.frame(list(low = data_object$lower[,i][time_range_index], 
    ...
    plot <- plot %>% hc_add_series(
      ...
      tooltip = list(table = TRUE,
                     sort = TRUE,
                     pointFormat = paste0( '<br> <span style="color:{point.color}">\u25CF</span>',
                                           " {series.name}: ", tool_tip$prefix , "{point.low}-{point.high}", norm_factor_and_unit$unit, tool_tip$suffix),
                     headerFormat = '<span style="font-size: 13px">{point.key}</span>'
      )
    )
