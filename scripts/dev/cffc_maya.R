...
data <- c("example_data/ORD-551132-J1V4_Covid19 Wellbeing W2_MAIN_Final Data_SPSS_v1.sav",
          "example_data/NZ_Covid 19 Wellbeing 2 with components with serials.sav")
  ...
  for (j in 1:length(output_list[[i]])) {
    ...
    if (i <= length(output_list)-1) {
      ...
      if (all(levels(df_i[[1]]) != levels(df_ii[[1]]))) {
        #levels(df_i[[1]]) <- levels(order(df_ii[[1]], decreasing = TRUE))
        print(str(df_i))
        df_i <-df_i[order(df_i[[1]], decreasing = TRUE),]
        print(str(df_i))
        print(df_i)
