...
cols <- c("f67453", "#4c8286", "#6dbbc0", "#8ca14b", "#b0ca5e", "#ffd500", "#edb335", "#e75b42", "#c2204d", "#de3765")
```
...
fig1 <- plot_ly(
  ...
  marker = list(color = "#f6c653", line = list(color = "#ffa500")),
  hoverinfo = 'text',
  text = ~paste0(format(x = df_events$month,  "%B"), ': ', df_events$ind_count_cum)
)

fig1 <- fig1 %>% add_lines(x = df_events$month, y = df_events$views_count, name = "Monthly visits", yaxis = "y2", marker = list(color = "#de3765"))
fig1 <- fig1 %>% add_lines(x = df_events$month, y = df_events$download_count, name = "Monthly downloads", yaxis = "y2", marker = list(color = "#4c8286"))
...
fig <- fig %>% layout(title = "Number of indicators by frequency",  showlegend = TRUE,
...
### Sub-series
```{r}
vals <- c(sum(df_indicators$region), sum(df_indicators$gender), sum(df_indicators$age), sum(df_indicators$ethnic), sum(df_indicators$industry), sum(df_indicators$other), sum(df_indicators$no_subseries))
labs <- c("By region", "By sex/gender", "By age", "By ethnicity", "By industry", "Other sub-series", "No sub-series")
fig <- plot_ly(
  labels = labs, 
  values = vals, 
  textinfo='percent', 
  ...
  marker = list(colors = cols))
fig <- fig %>% add_pie(hole = 0.6)
fig <- fig %>% layout(title = "Number of indicators by sub-series",  showlegend = TRUE,
                      xaxis = list(showgrid = FALSE, zeroline = FALSE, showticklabels = FALSE),
                      yaxis = list(showgrid = FALSE, zeroline = FALSE, showticklabels = FALSE))
...
data <- df_indicators %>% 
  ...
  arrange(desc(count)) %>% 
  head(n = 10)
knitr::kable(data, col.names = c("Supplier", "Number of indicators"))
