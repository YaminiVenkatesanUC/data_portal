...
```{r}
data <- df_indicators %>% 
  select(indicator_name, no_subseries, region, gender, age, ethnic, industry, other) %>% 
  unique() %>% 
  mutate(sum = region + gender + age + ethnic + industry + other)
no_sub <- sum(data$no_subseries)
more_than_one <- data %>% 
  filter(sum > 1) %>% 
  summarise(count = n())
series <- data %>% 
  filter(sum <= 1)
subs <- df_events$ind_count_cum[length(df_events$ind_count_cum)] - sum(data$no_subseries)
vals <- c(no_sub,
          subs,
          more_than_one,
          sum(series$region),
